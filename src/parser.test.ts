...
describe('ini parser library', () => {
    ...
    it('parseIni should return an array' , () => {
        expect(parseIni(sampleIni, { allowGlobalSection: true, globalSectionName: 'global' })).to.be.an('array');
    });

    it('parseIni should return an array of length 2' , () => {
        expect(parseIni(sampleIni, { allowGlobalSection: true, globalSectionName: 'global' })).to.have.lengthOf(2);
    });

    it('parseIni should return an array of length 1' , () => {
        expect(parseIni(sampleIni, { allowGlobalSection: false })).to.have.lengthOf(1);
    });

    it('parseIni should return an array of length 1' , () => {
        expect(parseIni(sampleIni2, { allowGlobalSection: false })).to.have.lengthOf(1);
    });

    it('parseIni should have 4 blocks' , () => {
        expect(parseIni(sampleIni2, { allowGlobalSection: false })[0].blocks).to.have.lengthOf(4);
    });

    it('parseIni should have 10 blocks' , () => {
        expect(parseIni(sampleIni3, { allowGlobalSection: false })[0].blocks).to.have.lengthOf(10);
    });

    it('parseIni should have 2 blocks' , () => {
        expect(parseIni(sampleIni4, { allowGlobalSection: false })[0].blocks).to.have.lengthOf(2);
    });
    ...
    it('parseIni on sampleIni4 should have 0 blocks on global' , () => {
        expect(parseIni(sampleIni4, { allowGlobalSection: true, globalSectionName: 'global' })[0].blocks).to.have.lengthOf(0);
    });

    it('parseIni on sampleIni4 should have 2 blocks on DATABASE' , () => {
        expect(parseIni(sampleIni4, { allowGlobalSection: true, globalSectionName: 'global' })[1].blocks).to.have.lengthOf(2);
    });

    it('parseIni on sampleIni4 should have a section named DATABASE' , () => {
        const obj = parseIni(sampleIni4, { allowGlobalSection: true, globalSectionName: 'global' })
    ...
    it('parseIni on sampleIni4 should not have a section named global' , () => {
        const obj = parseIni(sampleIni4, { allowGlobalSection: false, globalSectionName: 'global' })
        expect(obj[0].section).to.not.equal('global');
    });

    it('parseIni on sampleIni4 should have a section named other' , () => {
        const obj = parseIni(sampleIni4, { allowGlobalSection: true, globalSectionName: 'other' })
